...
class MixedPrototype(ABC):
    ...
    def right_action_probs(self, actions, rewards, beta, cec_weight, *args):
        ...
        # args_list = list([*args])
        # #print("args_list[0]:", args_list[0].eval())

        # for i in range(len(args)):
        #     args[i] = at.as_tensor_variable(args[i])
    ...
    def fit(self, pid_num, block):
        #print("self.pid_list:", self.pid_list)
        #print("pid_num type:", type(pid_num))
        #print("self.pid_list[pid_num]:", self.pid_list[pid_num])
        
        pid = self.pid_list[pid_num]

        if self.save_path is not None:
            print('dir in question:', f'{self.save_path}/{self.name}/')
            print("does it exist?", os.path.exists(f'{self.save_path}/{self.name}/'))
            if not os.path.exists(f'{self.save_path}/{self.name}/'):
                print('Making new directory', f'{self.save_path}/{self.name}/')
